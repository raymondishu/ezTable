function initEzTable(angular,ezTablePatch) {
    var angularModule=angular.module('ezModule',[])
    angularModule.directive("dyCompile", ["$compile", function($compile) {
        return {
            replace: true,
            restrict: 'EA',
    ...
    angularModule.directive('ezDynamic',function () {
        return{
            scope:{
                field:'=field',
                item:'=item',
            ...
            controller:function ($scope,$element,$attrs) {
                    ...
                    console.log($element);
                    console.log($attrs);
                // $scope.value='<input type="button" ng-click="fn1(item[field])" value="{{item[field]}}">';
                if($scope.template){
                    $scope.value=$scope.template;
                }else{
                    $scope.value='<div>{{item[field]}}</div>';
                }
            ...
            template:"<div dy-compile html='{{value}}'></div>"

        }
    });
    angularModule.directive('tmPagination',[function(){
        return {
            restrict: 'EA',
    ...
    angularModule.directive('ezTable',function () {
        return{
            scope:{
                arr1:'=',
                fn1:'=',
            ...
            restrict:'EA',//指令类型
            replace:true,//替换原标签
            templateUrl:ezTablePatch+'/template/ezTable.html',
            controller:function($scope,$element,$attrs){
                $scope.sortClick=function (name) {
